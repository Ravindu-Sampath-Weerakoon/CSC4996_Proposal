...
\begin{enumerate}
    ...
    \item \textbf{To implement a lightweight anomaly detection model:} Develop a \textbf{\ac{Deep SVDD}} classifier \cite{ruff2018deepsvdd} capable of running offline on mid-range mobile devices to distinguish between genuine users and imposters with minimal computational overhead.
...
\begin{enumerate}
    ...
    \item \textbf{Feasibility for Mobile Environments:} Can a \textbf{\ac{Deep SVDD}} anomaly detection model achieve real-time authentication latency (e.g., $<100$ms) on mid-range mobile devices without exceeding memory constraints? 
\end{enumerate}
