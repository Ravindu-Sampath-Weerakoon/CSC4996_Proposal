...
\begin{figure}[H]
    ...
    \begin{scope}[xshift=8.5cm, local bounding box=PanelB]
        ...
        % Shading the area between straight and curve is complex in TikZ without intersections lib, 
        % so we label the deviation instead.
...
\begin{figure}[htbp]
    ...
    \begin{scope}[xshift=6.5cm, yshift=0.5cm] % Increased xshift for more gap
        % Draw 2D Plane
...
\begin{itemize}
    \item Biometric performance (EER, FAR, FRR)
    \item Computational efficiency (Inference Latency, Encryption Overhead Ratio)
